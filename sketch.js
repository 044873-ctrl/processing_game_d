...
function handleBlockCollisions(){for(let i=blocks.length - 1;i>=0;i--){let b=blocks[i];if(circleRectCollision(ball.x,ball.y,ball.r,b.x,b.y,b.w,b.h)){spawnParticles(b.x + b.w/2,b.y + b.h/2);blocks.splice(i,1);score += 10;ball.vy = -ball.vy;break;}}}
function drawBlocks(){for(let i=0;i<blocks.length;i++){let b=blocks[i];fill(b.color);rect(b.x,b.y,b.w,b.h);}}
function drawUI(){fill(255);text("Score: " + score,8,8);let elapsedMs=millis() - startTime;let elapsedSeconds=elapsedMs/1000;let timeStr=elapsedSeconds.toFixed(2);textAlign(RIGHT,TOP);text(timeStr,width-8,8);textAlign(LEFT,TOP);if(!gameOver){let intSec=Math.floor(elapsedSeconds);while(lastSpeedIncreaseSecond < intSec){lastSpeedIncreaseSecond++;ball.vx *= 1.2;ball.vy *= 1.2;ballSpeed *= 1.2;}}}
