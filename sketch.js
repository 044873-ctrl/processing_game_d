...
let marginX=20;
let gap=6;
let blockW=(width - marginX*2 - gap*(cols-1))/cols;
let blockH=20;
let startY=60;
for(let r=0;r<rows;r++){
for(let c=0;c<cols;c++){
let bx=marginX + c*(blockW+gap);
let by=startY + r*(blockH+gap);
let bcolor=blockColors[r%blockColors.length];
blocks.push({x:bx,y:by,w:blockW,h:blockH,color:bcolor});
}
}
}
