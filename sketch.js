...
function handlePlateCross(){let zoneTop=batterY-80;let zoneBottom=batterY-20;let withinZone=(pitchY>=zoneTop && pitchY<=zoneBottom);let closeToBat=abs(pitchX-batterX)<=40 && abs(pitchY-batterY)<=40;if(swinging && closeToBat){processHit();}else{if(withinZone){strikes++;if(strikes>=3){outs++;strikes=0;balls=0;checkInning();}}else{balls++;if(balls>=4){processWalk();}}} pitchActive=false;atBatDelay=nextPitchDelay;}
